...
impl Variant {
    pub fn apply_to(&self, composition: &mut Composition) {
        ...
        gpio(composition, gpio::Instance::A, rcc.ahb2enr.gpioaen);
        gpio(composition, gpio::Instance::B, rcc.ahb2enr.gpioben);
        gpio(composition, gpio::Instance::C, rcc.ahb2enr.gpiocen);
        gpio(composition, gpio::Instance::D, rcc.ahb2enr.gpioden);
        gpio(composition, gpio::Instance::E, rcc.ahb2enr.gpioeen);
        gpio(composition, gpio::Instance::F, rcc.ahb2enr.gpiofen);
        syscfg(composition, rcc.apb2enr.syscfgen);
        exti(composition);
        cordic(composition, rcc.ahb1enr.cordicen);
        crc(composition, rcc.ahb1enr.crcen);
        vrefbuf(composition, rcc.apb2enr.syscfgen);
        dma(
            composition,
            dma::Instance::Dma1,
            self.dma_channels(),
        ...
        dmamux(composition, 2, self.dma_channels(), rcc.ahb1enr.dmamux1en);
    }

    pub fn extra_interrupts(&self) -> bool {
        match self {
            Self::G431 | Self::G441 => false,
            Self::G474 | Self::G484 => true,
        }
    }

    pub fn dma_channels(&self) -> u8 {
        match self {
            Self::G431 | Self::G441 => 6,
            Self::G474 | Self::G484 => 8,
        }
    }
}
